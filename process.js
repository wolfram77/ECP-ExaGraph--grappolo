...
const RGRAPH = /^Input File\s*:\s*.*\/(.*?)\.mtx/m;
const RORDER = /^\|S\|= (.+?), \|T\|= (.+?), \|E\|= (.+)/m;
const RMODLT = /^Final modularity\s*:\s*(.+)/m;
const RCLSTR = /^Total time for clustering\s*:\s*(.+)/m;
const RRESLT = /^TOTAL TIME\s*:\s*(.+)/m;
...
function readLogLine(ln, data, state) {
  ...
  if (RGRAPH.test(ln)) {
    ...
    state.clustering_time   = 1000 * parseFloat(clustering_time);
  }
  else if (RRESLT.test(ln)) {
    var [, total_time] = RRESLT.exec(ln);
    data.get(state.graph).push(Object.assign({}, state, {
      total_time: 1000 * parseFloat(total_time),
    }));
  }
  return state;
}
